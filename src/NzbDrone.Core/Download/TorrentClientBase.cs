...
namespace NzbDrone.Core.Download
{
        ...
        where TSettings : IProviderConfig, new()
    {
        ...
        protected abstract string AddFromMagnetLink(RemoteItem remoteItem, string hash, string magnetLink);
        protected abstract string AddFromTorrentFile(RemoteItem remoteItem, string hash, string filename, byte[] fileContent);

        public override string Download(RemoteItem remoteMovie)
        {
            ...
            if (PreferTorrentFile)
            {
                ...
                if (magnetUrl.IsNotNullOrWhiteSpace())
                {
                    ...
                    {
                        throw new ReleaseDownloadException(remoteMovie.Release,
                            "Magnet not supported by download client. ({0})", ex.Message);
                    }
                }
            }
            else
        ...
        private string DownloadFromWebUrl(RemoteItem remoteEpisode, string torrentUrl)
        {
        ...
        private string DownloadFromMagnetUrl(RemoteItem remoteEpisode, string magnetUrl)
        {
            string hash = null;
